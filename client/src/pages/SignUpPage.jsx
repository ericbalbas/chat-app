...
const SignUpPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!email.trim()) return toast.error("Email is not required!");
    if (!password.trim()) return toast.error("Password is not required!");
    if (!password.length > 6)return toast.error("Password must be at least 6 characters!");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
      return toast.error("It is not an email!");
